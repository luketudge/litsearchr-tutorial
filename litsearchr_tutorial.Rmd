...
And just as a check, let's can take a look at the title of the first result.
...
Our list of new search terms is looking fairly good. But there are probably still some in there that are unrelated to the others and to our topic of interest. These perhaps only occur in a small number of articles that do not mention any of the other search terms. We would like some systematic way of identifying these 'isolated' search terms.
...
The rows of our matrix represent the articles (their titles and abstracts), and the columns represent the search terms. Each entry in the matrix records how many times that article contains that term. For example, if we look at the first five articles we see that *anxiety disorder* occurs twice each in the first two, and once in the third, and that *cognitive behavioural* occurs once in the first article.

```{r}
as.matrix(dfm)[1:5, 1:2]
```
...
```{r}
g <- create_network(as.matrix(dfm), min_studies=3)
```
...
Terms that appear near the center of the graph and that are linked to each other by darker lines are probably more important for our overall topic. Here these include *anxiety disorder*, *social phobia*, and two variants of *cognitive-behavioral therapy* (with and without a hyphen), among others.

Terms that appear at the periphery of the graph and linked to it only by faint lines are not closely related to any other terms. These are mostly tangential terms unrelated to our main topic, for example *fmri*, *autism*, and *virtual reality*.

Interestingly, we see one major anomaly at the very top of the graph. The clearly very relevant term *cognitive-behavioural therapy* is nonetheless isolated from the others. This is probably because it contains the minority British spelling of *behaviour*. We will need to treat this as a special case later on, and it is a good illustration of the importance of looking carefully at our results.
...
The figure shows the terms in ascending order of strength from left to right. Again, only an arbitrary subset of the terms are labeled, so as not to clutter the figure. We can see for example that there are four terms (starting with *anxiety*) that have much higher strengths than the others.
...
Now that we have got a revised list of search terms from the results of our naive search, we want to turn them into a new search query that we can use to get more articles relevant to the same topic. For this new, hopefully more rigorous, search we will need a combination of `OR` and `AND` operators. The `OR` operator should combine search terms that are all about the same subtopic, so that we get articles that contain any one of them. The `AND` operator should combine these groups of search terms so that we get only articles that mention at least one term from all of the subtopics that are interested in.
...
```{r}
grouped_terms <-list(
  medication=selected_terms[c(6, 23)],
  cbt=selected_terms[c(5, 13, 24, 25, 26)],
  phobia=selected_terms[c(1, 2, 4, 7, 8, 11, 16, 19)]
)
...
* `closure` controls whether these partial matches are matched at the left end of a word (`"left"`), at the right (`"right"`), only as exact matches (`"full"`) or as any word containing a word (`"none"`).
...
Let's read in the text file that we just wrote.

```{r}
read_file("search-inEnglish.txt")
```
...
I ran our new search and downloaded them to a file called *pubmed-pharmacoth-set.nbib*. Let's load this file of results with `litsearchr`.

```{r}
new_results <- import_results(file="pubmed-pharmacoth-set.nbib")
```

How many did we get this time?

```{r}
nrow(new_results)
```

We get a lot more than with the naive search. We now need to check whether the new results are relevant to our chosen topic. This is something that we cannot do automatically. At this stage, we have to start sifting through the results and apply our domain expertise. But there are a few basic things that we can check.
...
The good news here is that most of the articles that were in the naive search but not in the new search are not really relevant to the overall topic. For example, the first article is about dizziness, and many of the others are about clinical syndromes other than phobia.

The bad news is that there are some relevant articles that were in the naive search but are not in the new search. For example, there are clearly relevant studies of the drugs *moclobemide* and *D-cycloserine*. We should try to find out why we missed these. Some testing and checking with further searches will be necessary.
...
check_recall(important_titles, new_results[, "title"])
```
